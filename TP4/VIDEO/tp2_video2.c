		...
		if(cpt_frame >= 400 && cpt_frame < 800)
		{
			...
			for(i=1; i<height-1; i++)
			{
				for(j=1; j<width-1; j++)
				{
					...
					pixel_blue_no = frame1[(i+1)*line_size+3*(j-1)+RED];
					pixel_blue_n  = frame1[(i+1)*line_size+3*(j  )+RED];
					pixel_blue_ne = frame1[(i+1)*line_size+3*(j+1)+RED];
					pixel_blue_so = frame1[(i+1)*line_size+3*(j-1)+RED];
					pixel_blue_s  = frame1[(i+1)*line_size+3*(j  )+RED];
					pixel_blue_se = frame1[(i+1)*line_size+3*(j+1)+RED];
					pixel_blue_o  = frame1[(i  )*line_size+3*(j-1)+RED];
					pixel_blue_e  = frame1[(i  )*line_size+3*(j+1)+RED];

					pixel_green_no = frame1[(i+1)*line_size+3*(j-1)+GREEN];
					pixel_green_n  = frame1[(i+1)*line_size+3*(j  )+GREEN];
					pixel_green_ne = frame1[(i+1)*line_size+3*(j+1)+GREEN];
					pixel_green_so = frame1[(i+1)*line_size+3*(j-1)+GREEN];
					pixel_green_s  = frame1[(i+1)*line_size+3*(j  )+GREEN];
					pixel_green_se = frame1[(i+1)*line_size+3*(j+1)+GREEN];
					pixel_green_o  = frame1[(i  )*line_size+3*(j-1)+GREEN];
					pixel_green_e  = frame1[(i  )*line_size+3*(j+1)+GREEN];

					pixel_red_no  = frame1[(i+1)*line_size+3*(j-1)+BLUE];
					pixel_red_n   = frame1[(i+1)*line_size+3*(j  )+BLUE];
					pixel_red_ne  = frame1[(i+1)*line_size+3*(j+1)+BLUE];
					pixel_red_so  = frame1[(i+1)*line_size+3*(j-1)+BLUE];
					pixel_red_s   = frame1[(i+1)*line_size+3*(j  )+BLUE];
					pixel_red_se  = frame1[(i+1)*line_size+3*(j+1)+BLUE];
					pixel_red_o   = frame1[(i  )*line_size+3*(j-1)+BLUE];
					pixel_red_e   = frame1[(i  )*line_size+3*(j+1)+BLUE];
					...
					{
						sobel[i*(width*3)+3*j+BLUE] = 255;
						sobel[i*(width*3)+3*j+GREEN] = 255;
						sobel[i*(width*3)+3*j+RED] = 255;
					}
					else
					{
						sobel[i*(width*3)+3*j+BLUE] = 0;
						sobel[i*(width*3)+3*j+GREEN] = 0;
						sobel[i*(width*3)+3*j+RED] = 0;
					}
				}
			}
			...
			for(i=1; i<height-1; i++)
			{
				for(j=1; j<width-1; j++)
				{
					...
					frame1[i*line_size+3*j+BLUE] = sobel[i*(width*3)+3*j+BLUE];
					frame1[i*line_size+3*j+GREEN] = sobel[i*(width*3)+3*j+GREEN];
					frame1[i*line_size+3*j+RED] = sobel[i*(width*3)+3*j+RED];
